  ...
  std::vector<unsigned char> & shifted)
{
  ...
  double hueshift;
  if (shift<-180.0){
    hueshift = (shift + 360.0) + 180.0;
  }else if(shift>=180.0){
    hueshift = (shift - 360.0) + 180.0;
  }else{
    hueshift = shift + 180.0; //otherwise just have it store the shift value, as it means it contains a value in the correct range. then we just add the 180 to have it in range [0, 360)
  }
  ...
  //now, we are going to extract the values of r g and b channels for each pixel
    ...
    for (int row=0; row<height; row++){
      for (int col=0; col<width; col++){
        ...
        //step 6
        shifted[pixel_ind + 0] = static_cast<unsigned char>(new_red);
        shifted[pixel_ind + 1] = static_cast<unsigned char>(new_green);
        shifted[pixel_ind + 2] = static_cast<unsigned char>(new_blue);
